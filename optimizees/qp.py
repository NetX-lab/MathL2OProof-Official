...
class QP(BaseOptimizee):
    ...
    def __init__(
        ...
        **options
        ...
        """
        The QP optimization problem is formulated as

            minimize (1/2) * ||Y - W @ X||_2^2
               X
        """
        self.device = device
        self.vars = dict()
        ...
        if ood:
            ...
            self.x0_shift = torch.zeros(1, dtype=dtype, device=device)
            self.fx_shift = torch.zeros(1, dtype=dtype, device=device)
        ...
        if W is None:
            ...
            W = torch.randn(self.batch_size, self.output_dim,
                            self.input_dim, dtype=dtype).to(self.device)
            ...
            self.W = W / torch.sum(W**2, dim=1, keepdim=True).sqrt() # DO Not Let ||W^TW||_2 = 1 !!
    ...
    def initialize(self, seed):
        ...
        self.set_var('Z', torch.zeros(self.batch_size, self.input_dim,
                     1, requires_grad=True).to(self.device) + self.x0_shift)
        # smooth_subgrad = self.get_grad(
        #     grad_method='smooth_grad',
        #     compute_grad=True,
        #     retain_graph=False
        # )
        nonsmooth_subgrad = self.get_grad(
            grad_method='subgrad_nonsmooth',
            compute_grad=True,
            retain_graph=False
        )
        # self.set_var('c_k', (smooth_subgrad + nonsmooth_subgrad[0]).to(self.device))
        # self.set_var('d_k', (smooth_subgrad + nonsmooth_subgrad[1]).to(self.device))
        # self.set_var('c_k_1', (smooth_subgrad + nonsmooth_subgrad[0]).to(self.device))
        # self.set_var('d_k_1', (smooth_subgrad + nonsmooth_subgrad[1]).to(self.device))
        # self.set_var('z_k', torch.zeros(self.batch_size, self.input_dim, 1).to(self.device))
        self.set_var(
            'g_k', 0.5*(nonsmooth_subgrad[0] + nonsmooth_subgrad[1]).to(self.device))
        # self.set_var('grad_f_x_k_1', smooth_subgrad.to(self.device))
        # self.set_var(
        #     'g_k', torch.zeros(self.batch_size,
        #                        self.input_dim, 1, requires_grad=True).to(self.device))
        # self.set_var('V', smooth_subgrad.to(self.device))
        # self.set_var('G', 0.5*(nonsmooth_subgrad[0] + nonsmooth_subgrad[1]).to(self.device))
        ...
        # subgrad = self.get_grad(
        #     grad_method='subgrad',
        #     compute_grad=True,
        #     retain_graph=False
        # )

        # self.set_var('GM', (subgrad * 0.9999 / self.grad_lipschitz()).to(self.device))
        # self.set_var('VG', (subgrad * 0.9999 / self.grad_lipschitz()).to(self.device))

        self.set_var('V', torch.zeros(self.batch_size,
                     self.input_dim, 1, requires_grad=True).to(self.device))
        self.set_var('G', torch.zeros(self.batch_size,
                     self.input_dim, 1, requires_grad=True).to(self.device))
        self.set_var('C', torch.zeros(self.batch_size,
                     self.input_dim, 1, requires_grad=True).to(self.device))

        # self.set_var('V', torch.zeros(self.batch_size, self.input_dim, 1, requires_grad=True).to(self.device))
        self.set_var('B', torch.zeros(self.batch_size,
                     self.input_dim, 1, requires_grad=True).to(self.device))
        self.set_var('b4', torch.zeros(self.batch_size,
                     self.input_dim, 1, requires_grad=True).to(self.device))
        self.set_var('b5', torch.zeros(self.batch_size,
                     self.input_dim, 1, requires_grad=True).to(self.device))
    ...
    def objective(self, inputs: dict = None, compute_grad: bool = False):
        ...
        with torch.set_grad_enabled(compute_grad):
            residual = torch.matmul(self.W, X+self.fx_shift) - self.Y
            l2 = 0.5 * (residual**2.0).sum(dim=(1, 2)).sum() # sum loss, align with paper
    ...
    def objective_shift(self, inputs: dict = {}, compute_grad: bool = False):
        if inputs is None:
            inputs = {}
        return self.objective_batch_shift(inputs, compute_grad).sum() # sum loss, align with paper
    ...
    def smooth_grad(self, inputs: dict = None, compute_grad: bool = False, **kwargs):
        ...
        with torch.set_grad_enabled(compute_grad):
            residual = torch.matmul(W, X+self.fx_shift) - Y
            return torch.matmul(W.permute(0, 2, 1), residual)
    ...
    def subgrad_set(self, inputs: dict = None, compute_grad: bool = False, **kwargs):
        ...
        with torch.set_grad_enabled(compute_grad):
            residual = torch.matmul(W, X+self.fx_shift) - Y
            s_grad = torch.matmul(W.permute(0, 2, 1), residual)
            value = self.rho * torch.sign(X+self.fx_shift)
            signal = 1 - torch.sign(torch.abs(value))
    ...
    def prox(self, inputs: dict, compute_grad: bool = False, **kwargs):
        ...
        # TODO Double check OOD verion for smooth GD case.
        return -self.fx_shift + inputs['X']
    ...
    def load_from_file(self, path, start_index=0, size=32):
        ...
        self.W = torch.from_numpy(mats['W']).type(self.dtype).to(
            self.device)[start_index:start_index+size]
        self.Y = torch.from_numpy(mats['Y']).type(self.dtype).to(
            self.device)[start_index:start_index+size]
        # rho = mats['rho']
        # while isinstance(rho, np.ndarray):
        #     rho = rho[0]
        self.rho = 0.0 # NOTE: For QP problem, no L1 non-smooth part, simplist implementation
